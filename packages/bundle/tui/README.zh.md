...
- **answerer 是行模式且阻塞 turn**——审批/ask-user answerer 经单拥有者行分发器读整行；在 warp session-share 下，`SharedSessionWriteToLongRunningCommands` 可能 gate 此行为，需改非阻塞 answerer（见[分析 note](../../../.agents/notes/proposed/architecture/2026-08-18-tui-terminal-product-analysis.zh.md) §8）。
