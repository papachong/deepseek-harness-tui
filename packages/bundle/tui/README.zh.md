...
stdin 每行承载一个任务（管道或键入）。bin 在 boot 前暂停 stdin，使管道写端的数据在异步 boot 中存活，然后在 boot 后创建 readline 接口并经 `for await` 循环排空——每行驱动一次 `agent.followup` turn。stdout 是终端 surface；诊断走 stderr。审批 answerer 与 ask-user provider 复用同一 readline 接口。
