...
2. **Piped stdin cannot drive the `<Prompt>`** — OpenTUI's `<input>` consumes raw-mode keypress events; a pipe generates none, so the keyless snapshot replay via `echo | bun lib/bin.js` boots the renderer but cannot drive a turn. Live validation needs a real TTY + `DEEPSEEK_API_KEY`. This is inherent to a raw-mode TUI (opencode has the same property); the 40-archived-snapshot acceptance is deferred until a pty-driven snapshot harness exists.
3. **Build wiring is manual** — the root `scripts/build.ts` never calls per-package scripts, so `build:view` is not yet wired into `pnpm run build`. Running `bun scripts/build-view.ts` after the spine build is the current contract; wiring it as a `runScript('build:view', …)` follow-up is a deferred cleanup.
4. **Tool card specialization deferred** — the card-union switch (terminal/diff/read/search/web) falls through to generic for v1; the store does not populate `callView`/`resultView` (no tool registry). A later pass wires the runner to call `presentCall`/`presentResult` and pass views to the store.
5. **`lib/view/*.js` is gitignored** — like the rest of `lib/`, the view bundle is a build artifact regenerated by `bun scripts/build-view.ts`; the `files[]` whitelist ships it on publish.
