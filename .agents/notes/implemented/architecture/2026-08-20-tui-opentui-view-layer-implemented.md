...
2. **Solid reactivity under Bun (RESOLVED)** — Bun resolves bare `solid-js` to `dist/server.js` (the SSR build) where `createRenderEffect` runs once and never subscribes. The fix is `bunfig.toml` with `preload = ["@opentui/solid/preload"]`: the preload's Bun runtime plugin intercepts `solid-js/dist/server.js` and serves the reactive `dist/solid.js` build, matching opencode's `bunfig.toml`. With this, `<For each>` re-evaluates on store updates and streamed assistant text renders. A live PTY spec (`tests/live-pty.spec.ts`) confirms: the model reply (`Hello!`) renders with ANSI SGR styling via the Solid store → `<markdown streaming>` path.
3. **Piped stdin cannot drive the `<Prompt>`** — OpenTUI's `<input>` consumes raw-mode keypress events; a pipe generates none. A python `pty.fork` harness (`tests/pty-harness.ts`) drives the bin: `onSubmit` fires, the agent loop runs (`turn/start` → `assistant/chunk` events probe-confirmed), and the reply renders. Live validation needs a real TTY + `DEEPSEEK_API_KEY` (the spec self-skips without the key).
